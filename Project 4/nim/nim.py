...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        """
        # print("fut", future_rewards)
        future_reward = 0
        # if future_rewards is None:
        #     future_rewards = 0
        # if type(future_rewards) is not int:
        #     future_rewards = max(future_rewards)
        # future_rewards) if len(future_rewards) == 1 else future_rewards[0]
    ...
    def choose_action(self, state, epsilon=True):
        ...
        """
        if not epsilon or np.random.random() > self.epsilon:
